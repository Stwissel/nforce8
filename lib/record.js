const Record = function (data) {
  ...
  for (let key in data) {
    ...
    if (lowerKey === 'attributes') {
      self.attributes = data[key];
    } else if (lowerKey === 'attachment') {
      self._attachment = data[key];
    } else {
      self._fields[lowerKey] = data[key];
      self._changed.push(lowerKey);
    }
  }
};
...
Record.prototype.hasChanged = function (field) {
  if (!this._changed || this._changed.length === 0) {
    ...
    return true;
  } else if (_.indexOf(this._changed, field.toLowerCase()) !== -1) {
    return true;
  }
  return false;
};
...
Record.prototype._getPayload = function (changedOnly) {
  ...
  let data = _.transform(this._fields, function (result, value, key) {
