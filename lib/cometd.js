...
class CometDClient extends EventEmitter {
  ...
  async _send(messages) {
    ...
    if (this._transport === 'websocket' && this._ws && this._ws.readyState === WebSocket.OPEN) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!isConnect) {
        ...
        const handler = async (event) => {
          ...
          const matching = responses.filter((r) => r.id === expectedId || !r.id);
  ...
  /**
   * Schedule a reconnection attempt after the retry interval.
   */
  _scheduleReconnect() {
    if (this._disconnecting) return;
